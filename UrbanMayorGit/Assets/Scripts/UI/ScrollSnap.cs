...
public class ScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
{
    ...
    void Start()
    {
        ...
        child = _container.GetChild(_currentPage).GetComponent<Transform>();
        _horizontal = true;
        _lerp = false;
        SetPagePositions();
        SetPage(startingPage);
        InitPageSelection();
        //SetPageSelection(startFromPage);
        _container.GetChild(startingPage).transform.localScale = new Vector3(1f, 1f, 1f);
        gameObject.GetComponent<RectTransform>().sizeDelta = new Vector2(280, 350);
    }
    ...
    private void SetPagePositions()
    {
        ...
        if (_horizontal)
        {
            ...
            offsetX = (width / 2)+100;
            // total width
            containerWidth = width * _pageCount + 50*_pageCount;
    ...
    public void OnBeginDrag(PointerEventData aEventData)
    {
		...
		Camera.main.GetComponent<CameraMovement>().dragingObj = true;
    ...
    public void OnEndDrag(PointerEventData aEventData)
    {
        ...
        Camera.main.GetComponent<CameraMovement>().dragingObj = false;
    ...
    public void OnDrag(PointerEventData aEventData)
    {
        ...
        child = _container.GetChild(GetNearestPage()).GetComponent<Transform>();
        nearestPage = GetNearestPage();
        //if (child.localPosition.x - _container.transform.position.x < 50 || Mathf.Abs((child.localPosition.x + _container.transform.position.x )) > -50)
        {
            // jezeli zmienil sie child
            //changeScaleOfBalloons(nearestPage);
        }
        
    }

    IEnumerator PopOut(Transform obj, float from, float to)
    {
        float now = from;

        while (now < to)
        {
            now += from * Time.deltaTime / 0.08f;
            child.localScale = new Vector3(now, now, 1);
    ...
    public void changeScaleOfBalloons(int page)
    {
        child = _container.GetChild(page).GetComponent<Transform>();
        if (prevPage != page)
        {
            StartCoroutine(PopOut(child, 0.4f, 1f));
            for (int i = 0; i < 17; i++)
            {
                if (i != page)
                    _container.GetChild(i).GetComponent<Transform>().localScale = new Vector3(0.4f, 0.4f, 1);
            }
            prevPage = page;
        }
    }
